    ...
    return (
        <View style={tw`mt-5 mb-12`}>
            {foods?.map(({ title, description, image, price, id }, index) => (
                <View style={tw`mb-3 flex-row justify-between items-center pb-3 border-b border-gray-100`} key={index} >
                    <View style={tw`flex-1 pr-3 flex-row items-center`}>
                        ...
                        <View style={tw`flex-1 pl-2`}>
                            <Text style={[tw`text-gray-900 font-bold mb-1`, { fontSize: 16 }]}>{title}</Text>
