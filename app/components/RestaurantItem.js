...
const RestaurantItemCard = ({ item, onPress }) => {
    ...
    return (
        <TouchableOpacity style={tw`mx-4 mb-4`} onPress={onPress}>
            ...
            />
            <TouchableOpacity style={tw`absolute top-2 right-2`} onPress={() => setLoved(e => !e)}>
                <Entypo name={`${loved ? 'heart' : 'heart-outlined'}`} size={28} color="#fff" />
            </TouchableOpacity>
            <View style={tw`flex-row items-center mt-1`}>
                <View style={tw`flex-grow`}>
                    <Text style={tw`font-bold text-lg`} numberOfLines={1}>{item.name}</Text>
                    <View style={tw`flex-row items-center`}>
                        <MaterialCommunityIcons name="clock-time-four" size={13} color="#06C167" />
                        <Text style={tw`text-xs text-gray-700`}> 20-30 • min • {item.price}</Text>
                    </View>
                </View>
